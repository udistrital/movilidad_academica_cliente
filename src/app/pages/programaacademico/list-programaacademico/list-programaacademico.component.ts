...
@Component({
  selector: 'ngx-list-programaacademico',
  templateUrl: './list-programaacademico.component.html',
  styleUrls: ['./list-programaacademico.component.scss'],
  })
export class ListProgramaacademicoComponent implements OnInit {
  ...
  cargarCampos() {
    this.settings = {
      ...
      columns: {
        ...
        Facultad: {
          title: this.translate.instant('GLOBAL.facultad'),
          // type: 'string;',
          valuePrepareFunction: (value) => {
            return value;
          },
  ...
  loadData(): void {
    this.movilidadAcademicaService.get('programaacademico/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.movilidadAcademicaService.delete('programaacademico/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
