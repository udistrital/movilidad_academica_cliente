...
@Component({
  selector: 'ngx-list-tipodocumento',
  templateUrl: './list-tipodocumento.component.html',
  styleUrls: ['./list-tipodocumento.component.scss'],
  })
export class ListTipodocumentoComponent implements OnInit {
  ...
  loadData(): void {
    this.movilidadAcademicaService.get('tipodocumento/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.movilidadAcademicaService.delete('tipodocumento/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'Tipodocumento deleted');
            }
         });
      }
    });
