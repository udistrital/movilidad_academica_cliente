...
export class ListOrganizacionComponent implements OnInit {
  ...
  loadData(): void {
    this.movilidadAcademicaService.get('organizacion/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.movilidadAcademicaService.delete('organizacion/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'Organizacion deleted');
            }
         });
      }
    });
