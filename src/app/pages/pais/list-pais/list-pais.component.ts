...
export class ListPaisComponent implements OnInit {
  ...
  loadData(): void {
    this.movilidadAcademicaService.get('pais/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.movilidadAcademicaService.delete('pais/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'Pais deleted');
            }
         });
      }
    });
